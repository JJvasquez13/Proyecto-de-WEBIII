...
async function showProjects(field, value) {
  ...
  try {
    const querySnapshot = await researchProjectsRef
      .where(field, ">=", value)
      .where(field, "<=", value + "\uf8ff")
      .get();
...
async function createProjectDiv(doc) {
  ...
  projectAreaOfInterest.innerHTML =
    "<strong>Area de Interes:</strong> " + doc.data().areaOfInterest;
  ...
  try {
    ...
    schoolGrade.innerHTML =
      "<strong>Grado de Colegio:</strong> " + doc.data().schoolGrade;
    projectDiv.appendChild(schoolGrade);

    var projectAuthor = document.createElement("div");
    projectAuthor.innerHTML = "<strong>Autor:</strong> " + studentName;
